...
calzados.forEach((calzado) => {
    ...
    card.innerHTML = `
        ...
        <div class="talles" id="talles-${calzado.id}"></div>
        ...
        calzado.talles.forEach((tal) => {
            const talles = document.createElement("div");
            talles.classList.add("tallesInd");
            talles.innerHTML=`
                <button class="botonTalle" id="botonTalle${tal}">${tal}</button>
                `
            document.getElementById(`talles-${calzado.id}`).append(talles);
        })


        const talleSelec = document.getElementById(`botonTalle${tal}`);

        talleSelec.onclick = () => {
            document.getElementById(`botonTalle${tal}`);
            talleSelec.class
        }
